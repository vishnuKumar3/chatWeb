...
router.post("/signup",function(req,res){
	...
	db.collection("authenticate").insertOne(formData,function(err,data){
		if(err) {console.log(err);return res.json({"status":false,"msg":"server side error"});}
			...
			defaultData={"_id":formData["_id"],"name":"user","contacts":[],"dp":"dp/defaultDP.png"};
			db.collection("users").insertOne(defaultData,function(err,data){
				if(err) {console.log(err);return res.json({"status":false,"msg":"server side error"});}
				else{
					console.log(data);return res.json({"status":true,"msg":"user added successfully"});
				}
			});
		}
	})
